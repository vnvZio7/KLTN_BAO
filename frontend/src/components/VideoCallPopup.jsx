...
export default function VideoCallPopup({ roomId, open, onClose, isDoctor }) {
  const appointmentId = roomId;
  const IS_DOCTOR = isDoctor || false;
  ...
  const doctorRecordingsRef = useRef([]);
  ...
  // ---------------------------------------------------------
  // Đảm bảo có session khi đủ 2 người
  // ---------------------------------------------------------
  async function ensureSession() {
    ...
    try {
      ...
      const res = await axiosInstance.post(API_PATHS.SESSIONS.CREATE_SESSION, {
        appointmentId,
      });
      console.log("Session created success :", res.data.session);
    } catch (err) {
  ...
  // ---------------------------------------------------------
  // Upload ALL recordings at END CALL
  // ---------------------------------------------------------
    ...
    try {
      ...
      blobs.forEach((blob, index) => {
        formData.append(
          "recordings",
          blob,
          `record-${appointmentId}-${Date.now()}-${index}.webm`
        );
      });

      // Debug xem formData có gì
      for (const [key, value] of formData.entries()) {
        console.log("FD:", key, value);
      }

      axiosInstance.patch(
        API_PATHS.SESSIONS.UPDATE_SESSION_BY_APPOINTMENT_ID(appointmentId),
        formData,
        {
          headers: {
  ...
  async function startDoctorRecording() {
    ...
    try {
      /** 1️⃣ Video source: screen nếu đang share, ngược lại dùng cam */
      let videoTracks = [];

      if (isScreenSharing && screenStreamRef.current) {
        videoTracks = screenStreamRef.current.getVideoTracks();
      ...
      if (localStreamRef.current) {
        audioTracks.push(...localStreamRef.current.getAudioTracks());
      }

      // audio bệnh nhân (remote)
      if (remoteVideo.current?.srcObject) {
        audioTracks.push(...remoteVideo.current.srcObject.getAudioTracks());
      }
      ...
      /** 4️⃣ MediaRecorder (KHÔNG dùng vp9) */
      ...
      rec.onstop = () => {
        ...
        doctorRecordingsRef.current.push(blob);
        ...
        console.log(
          "⏹️ Recording stopped, total blobs:",
          doctorRecordingsRef.current.length
        );
      };
  ...
  async function leaveCall() {
    ...
    if (r && r.state !== "inactive") {
      await new Promise((resolve) => {
        const handler = () => {
          r.removeEventListener("stop", handler);
          resolve();
        };
        r.addEventListener("stop", handler);
        r.stop();
      });
    }

    // 🔥 Upload tất cả recordings còn lưu
    await flushRecordingsToServer();
  ...
  // ------------------------------ //
  // Cleanup khi popup đóng/unmount //
  // ------------------------------ //
  useEffect(() => {
    if (!open) {
      cleanupCall();
